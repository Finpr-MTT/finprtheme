...
/**
 * Class Finprtheme_Product_Cart
 *
 * Handles WooCommerce checkout modifications for one-product checkout mode.
 */
class Finprtheme_Product_Cart {
    ...
    public function __construct() {
        add_action('init', [$this, 'finprtheme_setup_single_product_checkout_mode']);
        add_action('wp', [$this, 'finprtheme_remove_terms_and_conditions']);
    }

    /**
     * Setup single product checkout mode.
     */
    public function finprtheme_setup_single_product_checkout_mode() {
        // Disable add to cart messages.
        add_filter('wc_add_to_cart_message_html', '__return_false');

        // Remove previous product before adding a new one.
        //add_filter('woocommerce_add_cart_item_data', [$this, 'finprtheme_remove_previous_product'], 10, 2);

        // Empty the cart before adding a new product.
        add_filter('woocommerce_add_cart_item_data', [$this, 'finprtheme_empty_cart_before_adding_product']);

        // Redirect to checkout after adding product.
        add_filter('woocommerce_add_to_cart_redirect', [$this, 'finprtheme_redirect_to_checkout']);

        // Check for multiple products in the cart at checkout.
        //add_action('woocommerce_before_checkout_form', [$this, 'finprtheme_check_for_multiple_products']);

        // Disable order notes field.
        add_filter('woocommerce_enable_order_notes_field', '__return_false');

        // Change the order button text.
        add_filter('woocommerce_order_button_text', [$this, 'finprtheme_customize_order_button_text']);

        // Modify WooCommerce billing fields.
        add_filter('woocommerce_checkout_fields', [$this, 'finprtheme_modify_billing_fields']);

        // Hide specific countries on checkout.
        add_filter('woocommerce_countries', [$this, 'finprtheme_hide_countries_on_checkout'], 10, 1);
    }

    /**
     * Empty the cart before adding a new product.
     */
    public function finprtheme_empty_cart_before_adding_product($cart_item_data) {
     ...
     */
    public function finprtheme_remove_previous_product($cart_item_key, $product_id) {
     ...
     */
    public function finprtheme_redirect_to_checkout() {
     ...
     */
    public function finprtheme_check_for_multiple_products() {
     ...
     */
    public function finprtheme_refresh_cart_keep_last_product() {
     ...
     */
    public function finprtheme_customize_order_button_text() {
     ...
     */
    public function finprtheme_modify_billing_fields($fields) {
     ...
     */
    public function finprtheme_remove_terms_and_conditions() {
     ...
     */
    public function finprtheme_hide_countries_on_checkout($countries) {
